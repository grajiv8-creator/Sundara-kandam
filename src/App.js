...
export default function App() {
  ...
  useEffect(() => {
    ...
    setVerseData(null); setError(null); setLoading(true);
      ...
      <div style={S.root}>
        ...
        <div style={S.loginWrap}>
          <div style={S.loginCard}>
            ...
            {!showAddMember ? (
              <button onClick={() => setShowAddMember(true)} style={S.addMemberBtn}>+ Add Family Member</button>
            ) : (
              <div style={S.addMemberForm}>
                <input autoFocus value={newMemberName} onChange={e => setNewMemberName(e.target.value)} onKeyDown={e => e.key === "Enter" && addMember()} placeholder="Enter your name..." style={S.memberInput} />
    ...
    <div style={S.root}>
      ...
      <div style={S.nav}>
        {[["read", "📖 Read"], ["progress", "📊 Progress"], ["family", "👨‍👩‍👧 Family"]].map(([v, label]) => (
          <button key={v} onClick={() => setView(v)} style={{ ...S.navBtn, ...(view === v ? { ...S.navBtnActive, borderBottomColor: FAMILY_COLORS[member.colorIdx] } : {}) }}>{label}</button>
        ))}
      </div>
      ...
      {view === "read" && (
        <div style={S.readWrap}>
          <div style={S.padalamRow}>
            {PADALAMS.map(p => (
              <button key={p.id} onClick={() => { setCurPadalam(p.id); setCurVerse(1); }} style={{ ...S.padalamChip, background: curPadalam === p.id ? FAMILY_COLORS[member.colorIdx] : "#1a0a00", color: curPadalam === p.id ? "#fff8f0" : "#a07050", borderColor: curPadalam === p.id ? FAMILY_COLORS[member.colorIdx] : "#3a2010" }}>{p.id}</button>
            ))}
          </div>
          ...
          <div style={S.verseCard}>
            ...
            {loading && <div style={S.loadingWrap}><div style={S.spinner} /><div style={S.loadingText}>Loading pasuram...</div></div>}
            {error && <div style={S.errorWrap}><div style={S.errorText}>{error}</div><button onClick={() => { setError(null); setLoading(true); const p = PADALAMS.find(x => x.id === curPadalam); fetchPasuram(curPadalam, curVerse, p.name, p.nameEn, new AbortController().signal).then(d => { setVerseData(d); setLoading(false); }).catch(() => { setError("Failed. Please try again."); setLoading(false); }); }} style={S.retryBtn}>Try Again</button></div>}
            {verseData && !loading && (
              <>
                <div style={S.tamilSection}><div style={S.sectionLabel}>தமிழ்</div><div style={S.tamilText}>{verseData.tamilText}</div></div>
                <div style={S.dividerLine} />
                <div style={S.meaningSection}><div style={S.sectionLabel}>English Meaning</div><div style={S.meaningText}>{verseData.meaning}</div></div>
                {verseData.context && <div style={S.contextSection}><div style={S.contextText}>📍 {verseData.context}</div></div>}
              </>
            )}
          </div>
          {verseData && !loading && (
            <button onClick={markRead} style={{ ...S.markReadBtn, background: isRead ? "#1a3a1a" : FAMILY_COLORS[member.colorIdx], borderColor: isRead ? "#2a5a2a" : FAMILY_COLORS[member.colorIdx], color: isRead ? "#4ade80" : "#fff8f0" }}>
          ...
          {PADALAMS.map(p => {
            ...
            return (
              <div key={p.id} style={S.padalamProgressCard}>
                ...
                <div style={S.pPBarWrap}><div style={{ ...S.pPBar, width: `${pPct}%`, background: FAMILY_COLORS[member.colorIdx] }} /></div>
                <div style={S.pPStats}>{readInP} / {p.totalVerses} verses · {pPct}%</div>
                <div style={S.pPDots}>
                  {Array.from({ length: p.totalVerses }, (_, i) => {
                    const k = `${p.id}-${i + 1}`;
                    return <div key={k} onClick={() => { setCurPadalam(p.id); setCurVerse(i + 1); setView("read"); }} style={{ ...S.verseDot, background: memberProgress[k] ? FAMILY_COLORS[member.colorIdx] : "#2a1a08", border: (curPadalam === p.id && curVerse === i + 1) ? `1.5px solid ${FAMILY_COLORS[member.colorIdx]}` : "1.5px solid #3a2010", cursor: "pointer" }} />;
                  })}
                </div>
              </div>
            );
            ...
            return (
              <div key={m.id} style={S.familyCard}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={S.pPBarWrap}><div style={{ ...S.pPBar, width: `${fp}%`, background: FAMILY_COLORS[m.colorIdx] }} /></div>
                  <div style={S.familyStats}>{n} of {TOTAL_VERSES} verses · {fp}% complete</div>
                </div>
                {m.id !== activeMember && <button onClick={() => setActiveMember(m.id)} style={{ ...S.switchBtn, borderColor: FAMILY_COLORS[m.colorIdx], color: FAMILY_COLORS[m.colorIdx] }}>Switch</button>}
              </div>
            );
          })}
          <button onClick={() => setShowAddMember(true)} style={S.addMemberBtn2}>+ Add Family Member</button>
          {showAddMember && (
            <div style={S.addMemberForm}>
              <input autoFocus value={newMemberName} onChange={e => setNewMemberName(e.target.value)} onKeyDown={e => e.key === "Enter" && addMember()} placeholder="Enter name..." style={S.memberInput} />
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setShowAddMember(false)} style={S.cancelBtn}>Cancel</button>
...
const S = {
  ...
  readWrap: { padding: "16px", position: "relative", zIndex: 1, display: "flex", flexDirection: "column", gap: 14 },
